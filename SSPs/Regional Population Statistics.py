...
TIF_ROOT = r"F:\Article2\SSP_RCP\China_Clip\People"
OUT_CSV = r"F:\Article2\SSP_RCP\Result\People_sum_by_code.csv"
CODE_FIELD = "code"
ALL_TOUCHED = False
STRIP_EXT = False
FILL_IF_NONE = 0.0
...
def main():
    t0 = time.time()
    print("=== Job started (clamp negative pixels to 0; ensure unique pixel counting) ===")
    ...
    for idx, tif in enumerate(tif_paths, 1):
        ...
        try:
            with rio_open(tif) as src:
                ...
                if neg_count > 0:
                    print(f"  - Negative pixels found: {neg_count}. Clamped to 0.")
                    arr[neg_mask] = 0.0
