...
COUNTY_PATH = "Agriculture_Forestry_Husbandry_Fishery.csv"
CORR_PATH = "Corr_Result.csv"

OUT_ALLOC = "WA_agri_allocation.csv"
OUT_WEIGHTS = "WA_agri_weights.csv"
OUT_QUALITY = "WA_agri_quality.csv"
OUT_ALPHA = "WA_alpha_by_year.csv"
ERR_LOG = "WA_error.log"

INDUSTRY_KEY = "Agriculture_Forestry_Husbandry_Fishery"

W0 = {
    "Added_value_of_primary_industry": 0.25,
    "Agriculture_forestry_animal_husbandry_fishing": 0.30,
    "Per-capita_disposable_income_of_rural_residents": 0.05,
    "Cropland_Proportion": 0.10,
    "Forest_Proportion": 0.05,
    "Grassland_Proportion": 0.05,
    "Wetland_Proportion": 0.05,
    "Water_bodies_Proportion": 0.05,
    "Light_Data": 0.10,
}
...
GATE = dict(
    min_agri_land=0.02,
    penalty_low_land=0.2,
    high_water=0.80,
    penalty_high_water=0.1,
)
...
def soft_gate_row(row):
    if not SOFT_GATING:
        return 1.0
    cropland = float(row.get("Cropland_Proportion", 0.0) or 0.0)
    grass = float(row.get("Grassland_Proportion", 0.0) or 0.0)
    water = float(row.get("Water_bodies_Proportion", 0.0) or 0.0)
    g = 1.0
    if (cropland + grass) < GATE["min_agri_land"]:
        g *= GATE["penalty_low_land"]
    if water > GATE["high_water"]:
        g *= GATE["penalty_high_water"]
...
def load_inputs():
    ...
    if "Industry_Category" not in sum_df.columns:
        raise ValueError("Missing column in provincial file: Industry_Category")
    ...
    for e in energy_cols:
        sub[e] = pd.to_numeric(sub[e], errors="coerce").fillna(0.0)

    log(f"[INFO] Loaded: provincial={len(sum_df):,} rows (agri={len(sub):,}); county={len(cty_df):,} rows; energy_cols={len(energy_cols)}")
    return sub, cty_df, energy_cols

def ensure_groups(cty_df, sum_sub):
    pairs = set(zip(sum_sub["Province_ID"], sum_sub["Year"]))
...
def main():
    ...
    sum_sub, cty_df, energy_cols = load_inputs()

    feat_cols = [
        "Added_value_of_primary_industry",
        "Agriculture_forestry_animal_husbandry_fishing",
        "Per-capita_disposable_income_of_rural_residents",
        "Cropland_Proportion",
        "Forest_Proportion",
    ...
    cty_df = ensure_groups(cty_df, sum_sub)
    ...
    for gi, ((prov, year), gdf) in enumerate(grp, start=1):
        ...
        X["Light_Data"] = pd.to_numeric(X["Light_Data"], errors="coerce").fillna(0.0)
        ...
        tgt = sum_sub[(sum_sub["Province_ID"] == prov) & (sum_sub["Year"] == year)]
        if not tgt.empty:
    ...
    if not alloc_df.empty:
        targets = sum_sub.set_index(["Province_ID", "Year"])[energy_cols]
