...
COUNTY_PATH = "Transportation_Storage_Postal.csv"
CORR_PATH = "Corr_Result.csv"

OUT_ALLOC = "WA_transport_allocation.csv"
OUT_WEIGHTS = "WA_transport_weights.csv"
OUT_QUALITY = "WA_transport_quality.csv"
OUT_ALPHA = "WA_alpha_by_year.csv"
ERR_LOG = "WA_error.log"

INDUSTRY_KEY = "Transportation_Storage_Postal"
...
W0 = {
    ...
    "L_UNCLASSI": 0.010,
    "L_UNKNOWN": 0.010,
    "Urban_Proportion": 0.050,
    "Water_bodies_Proportion": 0.050,
    "Light_Data": 0.020,
}
...
def load_inputs():
    ...
    if sub.empty:
        cats = sum_df["Industry_Category"].dropna().unique()
        log("[ERROR] Filtering provincial file by INDUSTRY_KEY returned 0 rows. Sample available categories:")
        log(str(cats[:10]))
        raise ValueError("INDUSTRY_KEY does not match the provincial file. Please set it to the exact string in Industry_Category.")

    for e in energy_cols:
        sub[e] = pd.to_numeric(sub[e], errors="coerce").fillna(0.0)

    log(f"[INFO] Loaded: provincial={len(sum_df):,} rows ({INDUSTRY_KEY}={len(sub):,}); county={len(cty_df):,} rows; energy_cols={len(energy_cols)}")
...
def main():
    ...
    log("[INFO] Computing soft gate g (transport: urban/water)...")
    ...
    if not (abs(w0_vec.sum() - 1.0) < 1e-9):
        raise ValueError(f"W0 weights must sum to 1. Current sum={w0_vec.sum()}")
