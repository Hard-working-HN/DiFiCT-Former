...
COUNTY_PATH = "Residential_Life.csv"
CORR_PATH = "Corr_Result.csv"

OUT_ALLOC = "WA_residential_allocation.csv"
OUT_WEIGHTS = "WA_residential_weights.csv"
OUT_QUALITY = "WA_residential_quality.csv"
OUT_ALPHA = "WA_alpha_by_year.csv"
ERR_LOG = "WA_error.log"

INDUSTRY_KEY = "Residential_Life"

W0 = {
    "Registered_Population": 0.20,
    "Per-capita_Disposable_Income_of_Urban_Residents": 0.25,
    "Per-capita_disposable_income_of_rural_residents": 0.25,
    "Urban_Proportion": 0.05,
    "Cropland_Proportion": 0.05,
...
GATE = dict(
    min_settlement=0.02,
    penalty_low_settlement=0.2,
    very_low_urban=0.005,
    very_low_rural=0.01,
    penalty_very_low=0.1,
)
...
def soft_gate_row(row):
    ...
    crop = float(row.get("Cropland_Proportion", 0.0) or 0.0)
    grass = float(row.get("Grassland_Proportion", 0.0) or 0.0)
    settlement = urban + crop + grass
    g = 1.0
    if settlement < GATE["min_settlement"]:
        g *= GATE["penalty_low_settlement"]
    if (urban < GATE["very_low_urban"]) and ((crop + grass) < GATE["very_low_rural"]):
        g *= GATE["penalty_very_low"]
...
def load_inputs():
    ...
    log(f"[INFO] Loaded: provincial={len(sum_df):,} rows (residential={len(sub):,}); county={len(cty_df):,} rows; energy_cols={len(energy_cols)}")
...
def main():
    ...
    feat_cols = [
        ...
        "Per-capita_Disposable_Income_of_Urban_Residents",
    ...
    log("[INFO] Computing soft gate g (residential)...")
