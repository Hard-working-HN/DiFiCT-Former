...
COUNTY_PATH = "Wholesale_Retail_Accommodation_Catering.csv"
CORR_PATH = "Corr_Result.csv"

OUT_ALLOC = "WA_WRAC_allocation.csv"
OUT_WEIGHTS = "WA_WRAC_weights.csv"
OUT_QUALITY = "WA_WRAC_quality.csv"
OUT_ALPHA = "WA_alpha_by_year.csv"
ERR_LOG = "WA_WRAC_error.log"

INDUSTRY_KEY = "Wholesale_Retail_Accommodation_Catering"

W0 = {
    "Registered_Population": 0.20,
    "Total_retail_sales_of_consumer_goods_in_society": 0.40,
    "Per-capita_Disposable_Income_of_Urban_Residents": 0.20,
    "Urban_Proportion": 0.10,
    "Light_Data": 0.10,
}

SOFT_GATING = True
URBAN_GATE = dict(min_urban=0.01, penalty_low_urban=0.8)
...
def soft_gate_row_urban(row):
    if not SOFT_GATING:
        return 1.0
    urban = float(row.get("Urban_Proportion", 0.0) or 0.0)
    g = 1.0
    if urban < URBAN_GATE["min_urban"]:
        g *= URBAN_GATE["penalty_low_urban"]
...
def load_inputs():
    ...
    if ind_df.empty:
        ...
        raise ValueError(
            f"Industry '{INDUSTRY_KEY}' not found in provincial table. "
            f"Example available categories (first 10): {cats[:10]}"
        )

    for e in energy_cols:
        ind_df[e] = pd.to_numeric(ind_df[e], errors="coerce").fillna(0.0)

    log(f"[INFO] Loaded: provincial={len(sum_df):,} rows ({INDUSTRY_KEY}={len(ind_df):,}); county={len(cty_df):,} rows; energy_cols={len(energy_cols)}")
...
def read_alpha_from_corr_csv(path):
    ...
    if df.shape[1] < 2:
        raise ValueError("Correlation CSV must have at least two columns: Year and rho.")
...
def main():
    ...
    feat_cols = [
        ...
        "Total_retail_sales_of_consumer_goods_in_society",
    ...
    log("[INFO] Computing soft gate g (urban) ...")
    cty_df["__gate__"] = cty_df.apply(soft_gate_row_urban, axis=1).astype(float)
