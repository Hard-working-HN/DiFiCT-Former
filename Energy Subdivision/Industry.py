...
COUNTY_PATH = "Industry.csv"
CORR_PATH = "Corr_Result.csv"

OUT_ALLOC = "WA_industry_allocation.csv"
OUT_WEIGHTS = "WA_industry_weights.csv"
OUT_QUALITY = "WA_industry_quality.csv"
...
W0 = {
    "Number_of_industrial_units_above_designated_size": 0.30,
    "Gross_output_value_of_industrial_enterprises_above_designated_size": 0.30,
    "Investment_in_fixed_assets": 0.05,
    "General_budget_revenue_of_local_finance": 0.05,
    "General_budget_expenditure_of_local_finance": 0.05,
...
def soft_gate_row(row):
    ...
    bare = float(row.get("Bare_areas_Proportion", 0.0) or 0.0)
    g = 1.0
    if urban < GATE["min_urban"]:
        g *= GATE["penalty_low_urban"]
    if bare > GATE["high_bare"]:
        g *= GATE["penalty_high_bare"]
...
def load_inputs():
    ...
    if len(ind_df) == 0:
        raise ValueError(
            ...
            f"Available Industry_Category values: {cats}"
        )

    for e in energy_cols:
        ind_df[e] = pd.to_numeric(ind_df[e], errors="coerce").fillna(0.0)

    log(f"[INFO] Loaded: provincial={len(sum_df):,} rows (industry={len(ind_df):,}); county={len(cty_df):,} rows; energy_cols={len(energy_cols)}")
...
def read_alpha_from_corr_csv(path):
    ...
    log(f"[OK] Correlation CSV loaded and alpha generated: {os.path.abspath(OUT_ALPHA)}")
...
def main():
    ...
    log("[INFO] Computing soft gate g (industry semantics)...")
    ...
    for gi, ((prov, year), gdf) in enumerate(grp, start=1):
        ...
        def cb(w):
            ...
            if (iter_counter["n"] <= 5) or (iter_counter["n"] % ITER_PRINT_EVERY == 0):
                ...
                log(
                    f"    [ITER] ({prov}, {year}) it={iter_counter['n']} "
                    f"f={f:.6g} ce={ce:.6g} reg={reg:.6g} sum(w)={w.sum():.6f}"
                )
